...
describe('Testing the Interactions class', () => {
    ...
    it('will register and fire a click event and throw an error on a duplicate', done => {
        ...
        const { getByTestId } = render(<input data-testid={testID} onClick={e => actor.listen(e)} />);
        userEvent.click(getByTestId(testID));
        waitFor(() => {
            expect(_mockFire).toHaveBeenCalled();
            expect(() => actor.registerClickFunction(console.log)).toThrowError();
            done();
        })
    })
    it('will register and fire a key up event and throw an error on a duplication', done => {
        ...
        const { getByTestId } = render(<input data-testid={testID} onKeyUp={e => actor.listen(e)} />);
        userEvent.type(getByTestId(testID), 'r');
        waitFor(() => {
            expect(_mockFire).toHaveBeenCalled();
            expect(() => actor.registerKeyUpFunction('r', console.log)).toThrowError();
            done();
        })
    })
    it('will register and fire a key down event and throw an error on a duplication', done => {
        ...
        const { getByTestId } = render(<input data-testid={testID} onKeyDown={e => actor.listen(e)} />);
        userEvent.type(getByTestId(testID), 'r');
        waitFor(() => {
            expect(_mockFire).toHaveBeenCalled();
            expect(() => actor.registerKeyDownFunction('r', console.log)).toThrowError();
            done();
        })
    })
    ...
    it('will register multiple with a multiListener', done => {
        const [mockClick, mockKeyUp, mockKeyDown, mockEnter] = [null, null, null, null].map(() => jest.fn());
        const actor = new Interactions();
        ...
        const { getByTestId } = render(<input data-testid={testID} {...actor.multiListener('onClick', 'onKeyDown', 'onKeyUp')}/>);
        userEvent.click(getByTestId(testID));
